...
## 8. プロンプト管理方針（Prompt Logging Policy）

- Codex に与えられたプロンプト・追加指示は、今後必ず `.agents/PROMPTS.md` に原文のまま記述して保存すること。
- 新しい指示を受けた場合は、実装・改修と同時に `.agents/PROMPTS.md` へ追記し、後から経緯を確認できる状態を保つこと。
- 要約や翻訳ではなく、指示文の原文を保存することを優先する。必要に応じて日付や見出しを付けて整理する。

## 9. 変更履歴管理方針（Changelog Policy）

- Codex が変更をコミットする前に、必ず `.agents/CHANGELOG.md` を更新すること。
- `.agents/CHANGELOG.md` は時系列の最新順（新しい変更が上）で記述し、変更内容が後から追跡できるようにすること。
- 変更履歴には、主要な実装・ドキュメント更新・運用ルール変更を簡潔に記録すること。
